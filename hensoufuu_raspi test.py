...
while True:
    ...
    schoomy_data = ser.readline().decode('utf-8').rstrip()
    ...
    if target_object_count >= 1:
        ...
        if time.time() - last_send_time > MINIMUM_LINE_INTERVAL:
            ...
            line_message = "ご注文の商品の調理が完了しました："+str(last_send_time)

            # 検出物体の種別をメッセージに追加
            line_message += ' ' + "商品の重さ："+str(schoomy_data)
            ...
            files = {"imageFile": (
            file_name, attached_image, 'image/jpeg')}

            # ヘッダとデータ（ボディ）を指定してLINE通知APIサーバにPOSTリクエストを送信
            respo = requests.post(LINE_NOTIFY_API, headers=headers, data=body, files=files)
